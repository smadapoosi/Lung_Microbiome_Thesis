...
otu_matrix <- read.csv("~/PF_SEAFD_LOW_Caret_RF.dna.otu.caarsonly.normalized.csv")
...
#Feature F-Scores
f_score_calculator <- function(data) { #sourced from Chen and Lin 2011 (same implementation as libsvm in python)
	f_score_record <- NULL
	...
	write.csv(f_score_record, file = "~/OTU_NZV_F-Scores.csv", quote = F, row.names = F)
}
...
for (i in 1:100) {
	...
	feature_subset_sizes <- 0 #no recursive feature elimination
	...
	print(importance) #print only top 20 OTUs by F-AUC
	...
	file_path <- paste("~/PF_SEAFD_LOW_Caret_SVM_Plot_IT", iteration_number, "_CAARS_ONLY_Normalized.pdf", sep="")
	pdf(file_path)
...
write.table(roc_summary_test, file="~/PF_SEAFD_LOW_Caret_SVM_ROC_Testing_CAARS_ONLY_Normalized.txt", quote=F, row.names=F)
...
pdf("~/PF_SEAFD_LOW_Caret_SVM_ROC_Curve_Testing_CAARS_ONLY_Normalized.pdf")
